...
function renderEssayScreen(p, type) {
  ...
  // Stimulus — mirrors MCQ rendering: eyebrow label + italic title + text + attr
  if (p.stimulus) {
    const sbox = el('div', 'stimulus-box');

    // Eyebrow: "Primary Source · [source]" in small caps
    const slbl = el('span', 'stimulus-label', 'Primary Source · ' + (p.source || 'Practice Test'));
    ...
    // Document title in italic normal-case text
    if (p.stimulus.label) {
      const stitle = el('div', '', `<em>${p.stimulus.label}</em>`);
      stitle.style.cssText = 'font-size:12px;color:var(--ink-light);margin-bottom:8px;font-family:"Libre Baskerville",serif';
